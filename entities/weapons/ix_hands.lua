...
function SWEP:Think()
	...
	if (CLIENT) then
		...
		if (self:IsHoldingObject()) then
			...
			local targetLocation = self:GetOwner():GetShootPos() + self:GetOwner():EyeAngles():Forward() * holdDistance

			if (bIsRagdoll) then
				--targetLocation.z = Lerp( 0.5, targetLocation.z, self:GetOwner():GetShootPos().z - 32)
			end
				...
				if (SERVER and trace.Hit) then
					...
					if (IsValid(entity)) then
						local damageInfo = DamageInfo()
							...
							damageInfo:SetDamage(damage+testSubjectMult)
...
function SWEP:SecondaryAttack()
	...
	if (SERVER and IsValid(entity)) then
		...
		elseif (!entity:IsNPC() and self:CanHoldObject(entity)) then
			...
			if entity:GetNW2Bool( "isBMRPCorpse", false ) == true then
				hook.Run( "BMRP_BodyPickup", self:GetOwner(), entity )
			end
		end
	end
end
