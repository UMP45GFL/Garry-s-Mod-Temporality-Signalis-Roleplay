...
net.Receive("ciga",function()
	...
	elseif amt>=35 then
		local play = hook.Run("PlayerCigarBreath1Sound", ply)
		if play == nil or play then
			ply:EmitSound("cigabreath2.wav", 75, 100, 0.7)
		end

	elseif amt>=10 then
		local play = hook.Run("PlayerCigarBreath2Sound", ply, amt)
		if play == nil or play then
	...
	if ply.cigaArm != z then
		if z then
			timer.Simple(0.3, function() 
				if !IsValid(ply) then return end 
				if ply.cigaArm then
					local play = hook.Run("PlayerCigarInhaleSound", ply)
					if play == nil or play then
			...
			local stop = hook.Run("PlayerStopCigarInhaleSound", ply)
			if stop == nil or stop then
